...
<template>
    <MenuPageFrame :primary-color="category.color ?? undefined" :secondary-color="category.color_secondary ?? undefined">
        ...
        <div v-if="main" class="tc-mp-hero-row">
            ...
            <p class="tc-mp-price tc-mp-price--xl" :style="{ color: category.color_secondary ?? undefined }">${{ money(main.price) }}</p>
        </div>
        ...
        <div v-if="category.tagline" class="tc-mp-dot-divider" :style="{ color: category.color_secondary ?? undefined }">
            ...
            <span class="tc-mp-subtitle-text" :style="{ color: category.color_secondary ?? undefined }">{{ category.tagline }}</span>
            <span class="tc-mp-dot-divider-line" />
        </div>
        ...
        <div class="tc-mp-birria-sides">
            ...
            >
                ...
                <div class="tc-mp-alt-text">
                    <p class="tc-mp-name tc-mp-name--md" :style="{ color: category.color_secondary ?? undefined }">{{ side.name }}</p>
                    <p class="tc-mp-price tc-mp-price--sm" :style="{ color: category.color ?? undefined, '--tc-mp-h': category.color ?? undefined }">${{ money(side.price) }}</p>
                </div>
            </div>
        </div>
    </MenuPageFrame>
