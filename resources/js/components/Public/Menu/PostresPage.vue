...
<template>
    <MenuPageFrame :primary-color="category.color ?? undefined" :secondary-color="category.color_secondary ?? undefined">
        ...
        <template v-else>
            ...
            <p v-if="category.subtitle" class="tc-mp-choice" style="text-align: center" :style="{ color: category.color_secondary ?? undefined }">
                {{ category.subtitle }}
            </p>
        </template>
        ...
        >
            ...
            <div class="tc-mp-dessert-text">
                <p class="tc-mp-name" :style="{ color: category.color_secondary ?? undefined, fontSize: '1.3rem' }">{{ item.name }}</p>
                <p class="tc-mp-price" :style="{ color: category.color ?? undefined, '--tc-mp-h': category.color ?? undefined, fontSize: '1.6rem' }">${{ money(item.price) }}</p>
            </div>
        </div>
