...
const complementos = computed(() => byZone(props.category.items, 'complementos'));
...
<template>
    <MenuPageFrame :primary-color="category.color ?? undefined" :secondary-color="category.color_secondary ?? undefined">
        ...
        <div v-if="topTable.length" class="mt-4">
            ...
            <div v-for="item in topTable" :key="item.id" class="tc-mp-table-row">
                <span class="tc-mp-table-row-name">{{ rowLabel(item) }}</span>
                <span class="tc-mp-table-row-prices">
                    <span>${{ money(item.price) }}</span>
                    <span v-if="item.price_secondary">${{ money(item.price_secondary) }}</span>
                    <span v-else>&nbsp;</span>
                </span>
            </div>
        </div>
        ...
        <div v-if="conAlcohol.length" class="tc-mp-table-group mt-5">
            ...
            <div class="tc-mp-table-columns tc-mp-table-columns--split">
                <div v-for="item in conAlcohol" :key="item.id" class="tc-mp-table-row">
                    ...
                    <span class="tc-mp-table-row-prices"><span>${{ money(item.price) }}</span></span>
                </div>
            </div>
        </div>
        ...
        <div class="tc-mp-table-columns tc-mp-table-columns--2 mt-2">
            <div>
                <div class="tc-mp-table-group">
                    ...
                    <div v-for="item in cerveza" :key="item.id" class="tc-mp-table-row">
                        <span class="tc-mp-table-row-name">{{ item.name }}</span>
                        <span class="tc-mp-table-row-prices"><span>${{ money(item.price) }}</span></span>
                    </div>
                </div>
                <div class="tc-mp-table-group">
                    ...
                    <div v-for="item in sinAlcohol" :key="item.id" class="tc-mp-table-row">
                        <span class="tc-mp-table-row-name">{{ item.name }}</span>
                        <span class="tc-mp-table-row-prices"><span>${{ money(item.price) }}</span></span>
                    </div>
                </div>
            ...
            <div>
                <div class="tc-mp-table-group">
                    ...
                    <div v-for="item in complementos" :key="item.id" class="tc-mp-table-row">
                        <span class="tc-mp-table-row-name">{{ item.name }}</span>
                        <span class="tc-mp-table-row-prices"><span>${{ money(item.price) }}</span></span>
                    </div>
                </div>
                <div class="tc-mp-table-group">
                    ...
                    <div v-for="item in calientes" :key="item.id" class="tc-mp-table-row">
                        <span class="tc-mp-table-row-name">{{ item.name }}</span>
                        <span class="tc-mp-table-row-prices"><span>${{ money(item.price) }}</span></span>
                    </div>
                </div>
