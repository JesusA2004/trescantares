...
import { GripVertical, Pencil, Search, Star, Trash2, UtensilsCrossed } from 'lucide-vue-next';
...
props.categories.forEach((c) => {
    groups[String(c.id)] = props.items.filter((i) => i.menu_category_id === c.id);
});

const isFiltering = computed(() => filterCategory.value !== '' || filterSearch.value.trim() !== '');
...
return props.categories;
    ...
    return props.categories.filter((c) => String(c.id) === filterCategory.value);
});
...
const totalCount = computed(() => Object.values(groups).reduce((n, l) => n + l.length, 0));

const { registerZone, start, dragging, saving } = useDragSort<ItemRow>((zones) => {
...
<template>
    ...
    <div class="tc-admin-page space-y-5">
        ...
        <div class="tc-toolbar">
            ...
            <span v-if="saving" class="text-xs text-[var(--tc-blue)]">Guardando orden…</span>
            <span v-else class="text-xs text-gray-400 tabular-nums">{{ totalCount }} platillo{{ totalCount !== 1 ? 's' : '' }}</span>
        </div>

        <p v-if="isFiltering" class="text-xs text-amber-600 -mt-3">
            Quita los filtros para poder arrastrar platillos y moverlos entre categorías.
        </p>
        ...
        <div v-for="cat in visibleCategories" :key="cat.id" class="space-y-2">
            <h3 class="text-sm font-bold text-gray-700 flex items-center gap-2">
                {{ cat.name }}
                <span class="tc-badge tc-badge-gray text-[10px]">{{ visibleItems(cat.id).length }}</span>
            </h3>
                    ...
                    <tbody :ref="(el) => bindZone(cat.id, el as HTMLElement)">
                            ...
                            >
                                <td>
                                    <button
                                        ...
                                        @pointerdown="onHandleDown(item, cat.id, $event)"
                                    >
                                ...
                                <td class="hidden sm:table-cell text-gray-500 text-xs">{{ item.zone ?? '—' }}</td>
                        ...
                        <tr v-else>
                            <td colspan="8" class="text-center text-xs text-gray-400 py-6">
                                Arrastra un platillo aquí para moverlo a esta categoría.
                            </td>
                        </tr>
                    </tbody>
                </table>
