...
<template>
    ...
    <div class="tc-admin-page space-y-5">
        <AdminPageHeader
            title="Nuevo Usuario"
            description="Crea una cuenta de acceso al panel"
        >
            <template #label>Usuarios</template>
            <template #actions>
                <Link href="/admin/users" class="tc-btn-secondary"
                    >← Volver</Link
                >
            </template>
        </AdminPageHeader>
            ...
            >
                <div class="flex flex-wrap gap-2">
                    <button
                        ...
                        :key="role.id"
                        type="button"
                        class="tc-role-pill rounded-full border-2 px-3.5 py-1.5 text-sm font-medium transition-all"
                        :class="
                            form.roles.includes(role.name)
                                ? (roleActiveColors[role.name] ??
                ...
                <p
                    ...
                    class="text-xs text-[var(--tc-pink)]"
                >
                    {{ form.errors.roles }}
                </p>
            </AdminFormSection>
